...
(function ($) {
    ...
    function closeExternal(event) {
        ...
        if ((self.isLocked() && isOverMinWidth(self)) || $(event.target).parents('.' + self.options.classWrapper).length > 0 || $target.parents('.' + self.options.classWrapper).length > 0 || $target.hasClass('sidebar-swipe')) {
            return;
        }
    ...
    Sidebar.prototype.toggle = function (event) {
        ...
        if (event) {
            ...
            if ($target.hasClass(self.options.classToggle) || $target.parents('.' + self.options.classToggle).length > 0) {
                event.preventDefault();
            }

            if ($parents.length > 0 || $target.hasClass('sidebar-swipe')) {
                return;
            }
        }
